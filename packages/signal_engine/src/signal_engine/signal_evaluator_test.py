"""
Tests for SignalEvaluator - TDD RED stage.

These tests define the expected behavior of the signal_engine package.
...
def base_time() -> datetime:
    """Deterministic base timestamp for tests."""
...
@pytest.fixture
def evaluator(default_config: EngineConfig) -> SignalEvaluator:
    """SignalEvaluator with default configuration."""
    return SignalEvaluator(default_config)


@pytest.fixture
def hysteresis_evaluator(hysteresis_config: EngineConfig) -> SignalEvaluator:
    """SignalEvaluator with hysteresis enabled."""
    return SignalEvaluator(hysteresis_config)
...
class TestThresholdLogic:
    ...
    def test_ratio_above_threshold_triggers_alert(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """R1: When ratio > threshold, should_alert is True."""
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.40, now=base_time)
    ...
    def test_ratio_below_threshold_does_not_alert(
        ...
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.20, now=base_time)
    ...
    def test_ratio_equal_to_threshold_does_not_alert(
        ...
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.35, now=base_time)
    ...
    def test_ratio_just_above_threshold_triggers_alert(
        ...
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.350001, now=base_time)
    ...
    def test_threshold_boundary_parametrized(
        ...
        evaluator = SignalEvaluator(default_config)
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=ratio, now=base_time)
...
class TestNegativeRatios:
    ...
    def test_negative_ratio_does_not_alert(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """R6: Negative ratio (ask-heavy) should never trigger alert."""
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=-0.50, now=base_time)

        assert decision.should_alert is False

    def test_zero_ratio_does_not_alert(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """R6: Zero ratio (balanced) should not trigger alert (0 < 0.35)."""
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.0, now=base_time)

        assert decision.should_alert is False

    def test_very_negative_ratio_does_not_alert(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """R6: Extreme negative ratio (-1.0) should not trigger alert."""
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=-1.0, now=base_time)
    ...
    def test_non_positive_ratios_never_alert(
        self, default_config: EngineConfig, base_time: datetime, ratio: float
    ) -> None:
...
class TestCooldown:
    ...
    def test_second_alert_within_cooldown_is_suppressed(
        ...
        first = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)
        assert first.should_alert is True

        # Second evaluation within 30s cooldown
        within_cooldown = base_time + timedelta(seconds=15)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=within_cooldown)
    ...
    def test_alert_after_cooldown_expires_triggers(
        ...
        after_cooldown = base_time + timedelta(seconds=31)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=after_cooldown)
    ...
    def test_alert_exactly_at_cooldown_boundary_triggers(
        ...
        at_boundary = base_time + timedelta(seconds=30)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=at_boundary)
    ...
    def test_cooldown_resets_after_new_alert(
        ...
        # First alert at t=0
        evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)

        # Second alert at t=31s (after first cooldown)
        t31 = base_time + timedelta(seconds=31)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t31)
        assert second.should_alert is True

        # Third attempt at t=46s (15s after second alert, within new cooldown)
        t46 = base_time + timedelta(seconds=46)
        third = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t46)
...
class TestFirstEvaluation:
    ...
    def test_first_evaluation_for_symbol_has_no_cooldown(
        ...
        decision = evaluator.evaluate(symbol="DOTUSDT", ratio=0.50, now=base_time)
    ...
    def test_first_evaluation_below_threshold_does_not_alert(
        ...
        decision = evaluator.evaluate(symbol="DOTUSDT", ratio=0.20, now=base_time)
...
class TestIndependentSymbolState:
    ...
    def test_different_symbols_have_independent_cooldowns(
        ...
        # Alert for BTCUSDT
        btc_alert = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)
        assert btc_alert.should_alert is True

        # Immediately evaluate DOTUSDT - should alert (no cooldown for DOT)
        dot_alert = evaluator.evaluate(symbol="DOTUSDT", ratio=0.50, now=base_time)
        assert dot_alert.should_alert is True

        # BTC within cooldown should not alert
        btc_within_cooldown = evaluator.evaluate(
            symbol="BTCUSDT",
            ratio=0.50,
    ...
    def test_multiple_symbols_track_state_independently(
        ...
        evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t0)  # BTC alerts at t=0
        evaluator.evaluate(symbol="DOTUSDT", ratio=0.50, now=t5)  # DOT alerts at t=5
        evaluator.evaluate(symbol="SOLUSDT", ratio=0.50, now=t10)  # SOL alerts at t=10

        # At t=31: BTC cooldown expired (31s), DOT still in cooldown (26s), SOL in cooldown (21s)
        t31 = base_time + timedelta(seconds=31)
        btc_at_31 = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t31)
        dot_at_31 = evaluator.evaluate(symbol="DOTUSDT", ratio=0.50, now=t31)
        sol_at_31 = evaluator.evaluate(symbol="SOLUSDT", ratio=0.50, now=t31)
...
class TestHysteresis:
    ...
    def test_hysteresis_blocks_alert_until_ratio_drops_below_rearm_level(
        self, hysteresis_evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """R3: After alert, must drop below threshold - delta before re-arming."""
        # threshold=0.35, delta=0.10, so re-arm level = 0.25

        # First alert
        first = hysteresis_evaluator.evaluate(
            symbol="BTCUSDT", ratio=0.50, now=base_time
        )
        assert first.should_alert is True

        # After cooldown, but ratio still above re-arm level (0.30 > 0.25)
        after_cooldown = base_time + timedelta(seconds=35)
        still_high = hysteresis_evaluator.evaluate(
            symbol="BTCUSDT", ratio=0.30, now=after_cooldown
        )
    ...
    def test_hysteresis_allows_alert_after_ratio_drops_and_rises(
        ...
        # First alert at t=0
        hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)

        # At t=35s, ratio drops to 0.20 (below re-arm level 0.25) - re-arms
        t35 = base_time + timedelta(seconds=35)
        low = hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.20, now=t35)
        assert low.should_alert is False  # Below threshold, no alert

        # At t=36s, ratio rises above threshold again - should alert
        t36 = base_time + timedelta(seconds=36)
        high_again = hysteresis_evaluator.evaluate(
            symbol="BTCUSDT", ratio=0.50, now=t36
        )
    ...
    def test_hysteresis_exact_rearm_boundary(
        ...
        t35 = base_time + timedelta(seconds=35)
        at_rearm = hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.25, now=t35)
        assert at_rearm.should_alert is False  # Below threshold

        # Rise above threshold - should alert (re-armed at 0.25)
        t36 = base_time + timedelta(seconds=36)
        high = hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t36)
    ...
    def test_hysteresis_just_above_rearm_level_does_not_rearm(
        ...
        t35 = base_time + timedelta(seconds=35)
        hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.251, now=t35)

        # Try to trigger alert - should fail (not re-armed)
        t36 = base_time + timedelta(seconds=36)
        attempt = hysteresis_evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t36)
    ...
    def test_hysteresis_disabled_allows_immediate_realert_after_cooldown(
        ...
        t35 = base_time + timedelta(seconds=35)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=t35)
...
class TestEdgeCases:
    ...
    def test_extreme_positive_ratio(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
    ...
    def test_extreme_negative_ratio(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
    ...
    def test_very_small_positive_ratio(
        self, evaluator: SignalEvaluator, base_time: datetime
    ) -> None:
        """Edge: Very small positive ratio (0.001) should not alert (below threshold)."""
        decision = evaluator.evaluate(symbol="BTCUSDT", ratio=0.001, now=base_time)
        assert decision.should_alert is False

    def test_zero_cooldown_allows_consecutive_alerts(self, base_time: datetime) -> None:
        ...
        first = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)
        third = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=base_time)

        assert first.should_alert is True
        assert second.should_alert is True
        assert third.should_alert is True

    def test_very_high_threshold_rarely_alerts(self, base_time: datetime) -> None:
        ...
        moderate = evaluator.evaluate(symbol="BTCUSDT", ratio=0.80, now=base_time)
        assert moderate.should_alert is False

        extreme = evaluator.evaluate(symbol="BTCUSDT", ratio=0.96, now=base_time)
    ...
    def test_alert_decision_contains_correct_metadata(
        ...
        decision = evaluator.evaluate(symbol="SOLUSDT", ratio=0.42, now=base_time)
    ...
    def test_rapid_alternating_symbols(
        ...
        for i, symbol in enumerate(symbols):
            t = base_time + timedelta(milliseconds=i)
            decision = evaluator.evaluate(symbol=symbol, ratio=0.50, now=t)
            assert (
        ...
        for i, symbol in enumerate(symbols):
            t = base_time + timedelta(seconds=1, milliseconds=i)
            decision = evaluator.evaluate(symbol=symbol, ratio=0.50, now=t)
    ...
    def test_various_cooldown_durations(
        self, base_time: datetime, cooldown_seconds: float
    ) -> None:
        ...
        after_cooldown = base_time + timedelta(seconds=cooldown_seconds + 0.001)
        second = evaluator.evaluate(symbol="BTCUSDT", ratio=0.50, now=after_cooldown)
        assert second.should_alert is True
...
class TestRealisticScenarios:
    ...
    def test_multi_symbol_realistic_scenario(
        ...
        d_dot_0 = evaluator.evaluate(symbol="DOTUSDT", ratio=0.20, now=base_time)
        assert d_dot_0.should_alert is False

        # DOTUSDT rises above threshold at t=10
        d_dot_10 = evaluator.evaluate(
            symbol="DOTUSDT", ratio=0.40, now=base_time + timedelta(seconds=10)
        )
        assert d_dot_10.should_alert is True

        # BTCUSDT still in cooldown at t=20
        d_btc_20 = evaluator.evaluate(
            symbol="BTCUSDT", ratio=0.50, now=base_time + timedelta(seconds=20)
        )
        assert d_btc_20.should_alert is False

        # BTCUSDT cooldown expired at t=31
        d_btc_31 = evaluator.evaluate(
            symbol="BTCUSDT", ratio=0.50, now=base_time + timedelta(seconds=31)
        )
        assert d_btc_31.should_alert is True

        # DOTUSDT still in cooldown at t=35 (25s since t=10)
        d_dot_35 = evaluator.evaluate(
            symbol="DOTUSDT", ratio=0.50, now=base_time + timedelta(seconds=35)
        )
        assert d_dot_35.should_alert is False

        # DOTUSDT cooldown expired at t=41 (31s since t=10)
        d_dot_41 = evaluator.evaluate(
            symbol="DOTUSDT", ratio=0.50, now=base_time + timedelta(seconds=41)
        )
